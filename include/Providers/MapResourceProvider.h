...
/// Gets the Map from MapService when needed instead of storing a pointer
...
public:
	...
	MapResourceProvider(int cell_x, int cell_y, 
...
private:
	...
	/// Get the current map from MapService (or GameStatus for backward compatibility)
	/// @return Pointer to IMapResourceQuery, or nullptr if not available
	IMapResourceQuery* GetMapQuery() const;
